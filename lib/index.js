	...
	for (let i = 0; i < messaging_events.length; i++) {
		...
		if (event.message && event.message.text) {
			...
			if (productSearchActive && text != 'Please enter a product name to search for'){
				...
				console.log("Entered recipe search method");
				var headers1 = {
				   'X-Mashape-Key': 'RNsZ2tiy2pmshxmPkPRrn6viHrUwp1UZ22BjsnbuPVLyEnTS57',
				   'Accept': 'application/json'
				};
				console.log(text);
				var url1 = 'https://community-food2fork.p.mashape.com/search?key=9a266ac158d1966f7a00f043259f2c5e&q=';
				var url2 = url1.concat(text.trim());
				console.log(url);
				...
				function callback(error, response, body) {
						if (!error && response.statusCode == 200) {
								console.log(body);
								var resp1 = JSON.parse(body);
								recipes = resp1["recipes"];
								console.log(recipes[0]);
								console.log(resp1.count);
		...
		function callback(error, response, body) {
			if (!error && response.statusCode == 200) {
				...
				if(results.products.totalNumberOfResults != 0){
					...
					sendTextMessage(sendTo, "There are no results for that search.", token);
				}
			}
		}
	...
	let messageData = {
		"attachment": {
			...
			"payload": {
				...
				"elements": loadRecipeElements(ingredients) //Load the products into an object that messenger api can handle
			}
		}
	};
