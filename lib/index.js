...
app.post('/webhook/', function (req, res) {
	...
	for (let i = 0; i < messaging_events.length; i++) {
			...
			if (productSearchActive && text != 'Please enter a product name to search for'){
				...
				function callback(error, response, body) {
						if (!error && response.statusCode == 200) {
								...
								if(resp1.count != 0)
									sendRecipeMessage(sender, recipes);
								else{
									sendTextMessage(sender, "There are no results for that search.", token);
								}
						}
		...
		if (event.postback) {
			...
			if(event.postback.payload.contains("recipeSelected")){
...
function loadIngredients(id){
	...
	function callback(error, response, body) {
		if (!error && response.statusCode == 200) {
			...
			var resp = JSON.parse(body);
			resp1 = resp["recipes"];
			ingredients = resp1["ingredients"];
			console.log(ingredients[0]);
			console.log(ingredients.count);

			if(ingredients.count != 0)
				sendRecipeMessage(sender, ingredients[0]);
			else{
				sendTextMessage(sender, "There are no results for that search.", token);
			}
		}
...
function sendRecipeMessage(sender, recipe){
	let messageData = {
		"attachment": {
			"type": "template",
			"payload": {
