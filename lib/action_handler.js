...
function handleMessageReceive(sender, text) {
    ...
    // If the user is in the dictionary.
    if (net_handler.userStates.sender) {
        // If the user has requested to search for product by name..
        if (net_handler.userStates.sender == net_handler.State.WAITING_FOR_PRODUCT) {
            search.requestProduct(sender, text);

        // If the user has requested to search for a recipe..
        } else if (net_handler.userStates.sender == net_handler.State.WAITING_FOR_RECIPE) {
            recipe.requestRecipe(sender, text);
        }

    // If the user is currently idling (ie the server is not waiting for a search)..
    } else {
...
function handleButton(sender, payload) {
    ...
    switch (payload) {
        case "searchProduct":
            ...
            net_handler.userStates.sender = net_handler.State.WAITING_FOR_PRODUCT;
            break;

        case "searchRecipe":
            console.log("[EVENT] " + sender + ": Requested to search for recipes");
            sendTextMessage(sender, "What recipe are you looking for?");
            net_handler.userStates.sender = net_handler.State.WAITING_FOR_RECIPE;
            break;
    }
}
